/*
 * ESP32 C3 Mini - BLE Configuration & Web Server
 * 
 * This firmware allows an ESP32 C3 to be configured via BLE (Web Bluetooth)
 * and then serve a website from its internal flash storage.
 * 
 * Modes:
 * - BLE_CONFIG_MODE: Device advertises via BLE and accepts WiFi + file uploads
 * - WEB_SERVER_MODE: Device connects to WiFi and serves uploaded files via HTTP
 * 
...
void handleBLECommand(const char* jsonCommand) {
  ...
  if (strcmp(type, "wifi") == 0) {
  ...
  else if (strcmp(type, "relay_config") == 0) {
    ...
    // Add relay status
    statusDoc["relay_connected"] = relayConnected;
    if (publicNgrokUrl.length() > 0) {
      statusDoc["public_url"] = publicNgrokUrl;
    }
...
void handleRelayConfig(JsonDocument& doc) {
  ...
  if (!host || strlen(host) == 0) {
    sendStatus("error", "Missing relay server host");
    return;
  }
  ...
  relayServerHost = String(host);
  relayServerPort = port;
  relayServerPath = String(path);
  
  Serial.println("Relay configuration saved");
  sendStatus("success", "Relay config saved. Connecting...");
  
  // Connect to relay
  connectToRelay();
}

// ==================== Ngrok Relay Functions ====================

void connectToRelay() {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("WiFi not connected - cannot connect to relay");
    return;
  }
  
  if (relayServerHost.length() == 0) {
    Serial.println("No relay server configured");
    return;
  }
  ...
  switch(type) {
    ...
    case WStype_CONNECTED:
      ...
      // Register with relay
      {
        ...
        regDoc["deviceId"] = "ESP32-" + WiFi.macAddress();
        
        String output;
        serializeJson(regDoc, output);
    ...
    case WStype_TEXT:
      {
        ...
        if (strcmp(msgType, "registered") == 0) {
          publicNgrokUrl = doc["publicUrl"].as<String>();
          Serial.print("[WS] Registered! Public URL: ");
          Serial.println(publicNgrokUrl);
          
          // Send status update via BLE
          if (deviceConnected && pStatusCharacteristic != nullptr) {
            StaticJsonDocument<256> statusDoc;
            statusDoc["type"] = "relay_status";
            statusDoc["connected"] = true;
            statusDoc["public_url"] = publicNgrokUrl;
          ...
          Serial.print("[WS] HTTP ");
          Serial.print(method);
          Serial.print(" ");
          Serial.println(path);
          
          // Process request using web server logic
          String responseBody;
          int statusCode = 200;
          ...
          if (SPIFFS.exists(path)) {
            ...
            if (file) {
              responseBody = file.readString();
              contentType = getContentType(path);
              file.close();
            } else {
              statusCode = 500;
              responseBody = "Error reading file";
            }
          } else {
            statusCode = 404;
            responseBody = "File not found: " + path;
          }
          ...
          responseDoc["statusCode"] = statusCode;
          responseDoc["headers"]["Content-Type"] = contentType;
          responseDoc["body"] = responseBody;
...
void setup() {
  ...
  if (configured) {
    ...
    if (connectToWiFi()) {
      ...
      // Load and connect to relay server if configured
      preferences.begin("relay", true);
      relayServerHost = preferences.getString("host", "");
      relayServerPort = preferences.getInt("port", 80);
      relayServerPath = preferences.getString("path", "/");
      preferences.end();
...
void loop() {
  ...
  // Handle WebSocket relay connection
  if (relayServerHost.length() > 0 && WiFi.status() == WL_CONNECTED) {
    webSocket.loop();
  }
